  ...
  useEffect(() => {
    ...
    if (value && selectedH) {
      ...
      if (now.isBefore(dateSchedule)) {
        ...
        setErrorDate('La fecha y hora deben ser mayores a la actual por 2 minutos');
      }
    }
  }, [value, selectedH]);
    ...
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="md">
      <DialogContent sx={{ p: 5 }}>
        <Grid container spacing={2}>
          <Grid xs={12} md={6}>
            {smUp && <DateCalendar value={value} onChange={(newValue) => setValue(newValue)} />}
            {!smUp && sendButton}
          </Grid>
          ...
          <Grid xs={12} md={6}>
            ...
            >
              ...
              <Typography sx={{ fontSize: '14px', color: '#919EAB', textAlign: 'center' }}>
              ...
              <Stack spacing={2} direction="row" justifyContent="space-between">
                ...
                <LoadingButton
                  ...
                  onClick={() => {
                    ...
                    const dateH = dayjs(value).hour(Number(h)).minute(Number(m));
                    ...
                    handleClickSchedule({ date: dateH.utc().format() }).then((res: any) => {
                      if (res) {
                        setOpenSucces(true);
                      }
                    });
      ...
      <SendDialogSubs open={openSend} setOpen={setOpenSend} sendEmail={sendEmail} />
