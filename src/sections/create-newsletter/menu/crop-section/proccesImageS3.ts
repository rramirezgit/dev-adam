...
export const useProcessImagesS3 = () => {
  ...
  const currentNewsletterImagesList = useSelector(
    (state: RootState) => state.newsletter.currentNewsletterImagesList
  );
  ...
  const processSingleImageS3 = async (input: any) => {
    ...
    try {
      ...
      return replaceUrlImagesCurrentNewsletter({
        url: data.s3Url,
        key: data.filename,
        ...input,
      });
  ...
  const processImagesS3 = async () => {
    const inputsImages = currentNewsletter.filter((obj) =>
    ...
    dispatch(setCurrentNewsletterImagesList(imagesList));

    return replaceUrlImagesCurrentNewsletter(imagesList);
  };

  const replaceUrlImagesCurrentNewsletter = (image: any) => {
    const newCurrentNewsletter = currentNewsletter.map((obj) => {
